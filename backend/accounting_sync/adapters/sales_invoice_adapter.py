"""Adaptador `Invoice` (venta) ↔ `account.move` (out_invoice)."""
...
class SalesInvoiceAdapter:
    """Mapea `invoices.Invoice` con `account.move` (move_type=out_invoice)."""
    ...
    def _move_type(invoice: Any) -> str:
        """`out_refund` para nota de crédito, `out_invoice` para factura."""
        return 'out_refund' if invoice.invoice_type == 'CreditNote' else 'out_invoice'

    def _resolve_tax_ids(self, line: Any, company: Any) -> list[int]:
        """Devuelve la lista de account.tax IDs desde InvoiceLineTax.

        Si una línea tiene impuestos sin OdooTaxMapping, se loguea warning
        y se omite ese impuesto (no se duplica la línea).
        """
        tax_ids: list[int] = []
        for line_tax in line.taxes.all():
            ...
            if mapping:
                ...
                logger.warning(
                    'Línea %s sin OdooTaxMapping (sale) para tax_rate=%s '
    ...
    def _build_line(self, line: Any, company: Any) -> dict[str, Any]:
        ...
        # Descuento por línea: Odoo lo expresa como porcentaje
    ...
    def to_odoo(self, invoice: Any, *, company: Any) -> dict[str, Any]:
        """Construye el payload `account.move` desde una Invoice.

        Requiere:
            - `invoice.customer.odoo_id` poblado (push_contact previo).
            - Líneas con producto.odoo_id si se quiere enlace, opcional.

        Raises:
            ValueError: si el cliente no tiene `odoo_id`.
        """
        customer = invoice.customer
        if not customer.odoo_id:
            raise ValueError(
                f'Customer {customer.pk} sin odoo_id. '
                'Llama a push_contact() antes de push_sales_invoice().',
            )
        ...
        data: dict[str, Any] = {
            ...
            'ref': invoice.number or f'ERP-{invoice.pk}',
            'narration': invoice.customer_notes or False,
        }
        ...
        # Factura rectificativa: enlazar con la rectificada si tiene odoo_id
    ...
    def from_odoo(self, data: dict[str, Any]) -> dict[str, Any]:
        """Extrae campos relevantes de un `account.move` (uso futuro)."""
        return {
