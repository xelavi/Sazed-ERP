<template>
  <div class="quotes-view">
    <div class="view-header">
      <div class="header-content">
        <div class="title-section">
          <h1 class="view-title">Pressupostos</h1>
      ...
      <div class="filters-bar">
        <div class="search-box">
          ...
          <input
            ...
            placeholder="Cerca per nom o client…"
            v-model="searchQuery"
          />
        </div>
          ...
          <table class="table">
            ...
            <tbody>
              ...
              >
                ...
                <td>
                  <div class="contact-cell" v-if="q.customer">
                    <div class="avatar-sm" :style="{ background: q.customer.avatarColor }">
                      {{ q.customer.initials }}
                    </div>
      ...
      <div v-if="detailOpen && selectedQuote" class="drawer-overlay" @click.self="closeDetail">
        <div class="drawer-panel">
          ...
          <div class="drawer-body">
            <div class="info-row"><span>Client</span><strong>{{ selectedQuote.customer?.name }}</strong></div>
    ...
    <QuoteFormModal
      ...
      @close="closeForm"
      @save="handleSave"
      @contact-created="fetchCustomers"
    />
  </div>
</template>
...
import {
  mapSalesQuoteFromApi, mapSalesQuoteDetailFromApi, mapSalesQuoteToApi,
  mapCustomerFromApi, parseDrfErrors,
...
async function fetchQuotes() {
  try {
    ...
    quotes.value = items.map(mapSalesQuoteFromApi)
  } catch (err) {
...
onMounted(async () => {
  ...
  // Coming from Customers / Products to create a new quote prefilled
  if (route.query.newQuote === 'true') {
    if (route.query.customerId) {
      preselectedContactId.value = parseInt(route.query.customerId)
    }
...
async function handleSave(formData) {
  const apiData = mapSalesQuoteToApi(formData)
  try {
    if (formQuote.value) {
...
async function openDetail(q) {
  try {
    ...
    selectedQuote.value = mapSalesQuoteDetailFromApi(data)
...
async function convertToInvoice(q) {
  try {
    ...
    toast.success('Pressupost convertit en factura (esborrany)')
    closeDetail()
    await fetchQuotes()
  } catch (err) {
...
.invoice-link { color: var(--primary-color); font-weight: 600; font-family: monospace; }
