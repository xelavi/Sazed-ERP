...
const items = computed(() => {
  ...
  for (const inv of invitations.value) {
    list.push({
      ...
      body: `${inv.inviter?.full_name || inv.inviter?.email || 'Un administrador'} te ha invitado a unirte a ${inv.company_name} como ${roleLabel(inv.role)}.`,
      preview: `Rol propuesto: ${roleLabel(inv.role)}`,
